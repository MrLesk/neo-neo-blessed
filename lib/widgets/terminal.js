...
// Attempt to load node-pty only if it is present. When building with Bun
// we mark node-pty as an optional dependency so it may be absent.
var pty;
try {
  // Use eval to avoid static detection by bundlers when node-pty is absent.
...
Terminal.prototype.bootstrap = function() {
  ...
  if (pty) {
    ...
    // Provide a minimal stub so the rest of the widget does not crash.
    this.pty = {
      write: function() {},
      resize: function() {},
      on: function() {},
