...
/*
 * Built-in terminfo files inlined as Buffers so no external asset lookup is
 * required at runtime (helps Bun single-file bundles).
 */
const _builtinTerminfo = {
  'xterm': fs.readFileSync(require.resolve('../usr/xterm')),
  'xterm-256color': fs.readFileSync(require.resolve('../usr/xterm-256color')),
  'windows-ansi': fs.readFileSync(require.resolve('../usr/windows-ansi')),
  'linux': fs.readFileSync(require.resolve('../usr/linux'))
};
...
Tput.prototype._useXtermInfo = function() {
  const data = _builtinTerminfo['xterm'];
  const info = this.compile(this.parseTerminfo(data, '<inline:xterm>'));
  return this.inject(info);
};

Tput.prototype._useInternalInfo = function(name) {
  name = path.basename(name);
  // Static map so Bun can bundle the file paths.
  const data = _builtinTerminfo[name];
  if (!data) {
    throw new Error('Unknown built-in terminfo: ' + name);
  }
  // Parse Buffer directly and inject compiled info.
  const info = this.compile(this.parseTerminfo(data, '<inline:'+name+'>'));
  return this.inject(info);
};

Tput.prototype._useInternalCap = function(name) {
  name = path.basename(name);
  // Static map for termcap fallbacks.
  const capMap = {
    'xterm': require.resolve('../usr/xterm.termcap')
  };
  const file = capMap[name];
  if (!file) {
    throw new Error('Unknown built-in termcap: ' + name);
  }
  return this.injectTermcap(file);
};
...
Tput.prototype.readTerminfo = function(term) {
  ...
  if (_builtinTerminfo[term]) {
    info = this.parseTerminfo(_builtinTerminfo[term], '<inline:'+term+'>');
  } else {
