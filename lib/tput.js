...
Tput.prototype._useInternalInfo = function(name) {
  ...
  const infoMap = {
    'xterm': require.resolve('../usr/xterm'),
    'xterm-256color': require.resolve('../usr/xterm-256color'),
    'windows-ansi': require.resolve('../usr/windows-ansi'),
    'linux': require.resolve('../usr/linux')
  };
  const file = infoMap[name];
  if (!file) {
    throw new Error('Unknown built-in terminfo: ' + name);
  }
  return this.injectTerminfo(file);
};
